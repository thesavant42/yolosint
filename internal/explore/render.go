...
func renderManifestTables(w *jsonOutputter, m map[string]interface{}) error {
	...
	if cfg, ok := m["config"].(map[string]interface{}); ok {
		...
		// Build history link with icon - uses same URL pattern as config link
		w.Printf(` <a href="/%s%s@%s%smt=%s&size=%d&render=history"><img src="/material-symbols--network-intelligence-history.png" alt="history" style="height:16px;vertical-align:middle"/> build history</a>`,
			handler, w.repo, digest, qs, url.QueryEscape(mt), size)
	}
	...
	if layers, ok := m["layers"].([]interface{}); ok {
		for i, l := range layers {
			if layer, ok := l.(map[string]interface{}); ok {
				...
				w.Printf(`<tr><td>%d</td><td><a href="/%s%s@%s%smt=%s&size=%d&layer=%d&dlurl=%s">%s</a></td><td><a href="/size/%s@%s?mt=%s&size=%d">%s</a></td><td><a href="%s" download="%s" title="Download %s"><img src="/gis--layer-download.png" alt="Download" style="height:16px;vertical-align:middle"/></a></td></tr>`,
					i+1,
